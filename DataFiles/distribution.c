...
int main(int argc, char *argv[])
{
    ...
    for (arg = 1; arg < argc; arg++)
        ...
        if (sscanf(argv[arg], "d0=%lf %c", &dtemp, &dummy) == 1 ||
            ...
            sscanf(argv[arg], "q=%lf %c", &dtemp, &dummy) == 1 ||
            sscanf(argv[arg], "Q=%lf %c", &dtemp, &dummy) == 1) {
            p_diag_black = dtemp;
        } else {
    ...
    for (i = 1; i <= n; i++)
        ...
        if (c.white_histogram[i-1] || c.white_histogram[i] || c.white_histogram[i+1] ||
            ...
            euler = euler + (int64_t)(c.black_histogram[i]-c.white_histogram[i]); 
        }

    double euler_no = (double)euler/(double)c.iterations;
    printf("%.2lf: %lf\n",p_black, euler_no);
